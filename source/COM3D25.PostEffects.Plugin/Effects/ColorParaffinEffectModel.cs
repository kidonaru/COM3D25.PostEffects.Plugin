...
namespace COM3D25.PostEffects.Plugin
{
    public class ColorParaffinEffectModel : PostEffectModelBase
	{
		...
		public override void OnPreRender()
		{
			if (_computeBuffer == null)
			{
				// ストライドは構造体から導出する (MTE 原典の固定値 24 は実サイズ 22 float と不一致で、
				// Unity 2022.3 は SetData がストライド検証で例外になるため修正)
				_computeBuffer = new ComputeBuffer(MAX_PARAFFIN_COUNT,
					System.Runtime.InteropServices.Marshal.SizeOf(typeof(ParaffinBuffer)));
			}
