...
| `TonemappingColorGradingDef` | `_LUTEnabled`→`userLutEnabled` / `_contribution`→`userLutContribution` / `texture`→`userLutPath` / `_tonemappingCurve`・`_masterCurve`・`_redCurve`・`_greenCurve`・`_blueCurve`→`CurveData` |
