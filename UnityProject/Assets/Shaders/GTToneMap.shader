Shader "PostEffects/GTToneMap"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // C# 版 (GTToneMapController.EvaluateToneMap) と同式。片方を変えたらもう片方も追従させること
            float3 GTToneMap(float3 x) {
                float P = _MaxBrightness;
                float a = _Contrast;
                float m = max(_LinearStart, 1e-4); // 0 だと T_x の x/m がゼロ除算になる
                float l = min(_LinearLength, 0.999); // 1 だと C2 の分母 (P-S1) が 0 になる
                ...
                // 区間幅ゼロの H は step と等価 (H のままだとゼロ除算警告が出る)
